...
1. **Not a security boundary.** BasiliskShell reduces risk but should not be the sole control on a production system. Run with least-privilege user accounts.
2. **AI can be wrong.** Always read the command and explanation before confirming. Review is essential.
3. **No network sandboxing.** Commands that make network calls (e.g., `curl`) are permitted unless they pipe directly to `bash`/`sh`.
4. **Blocklist is not exhaustive.** Creative variants of dangerous commands may not be caught. Treat HIGH risk commands with extra scrutiny.
5. **Linux only.** Designed for Debian-based Linux distributions with `bash`, `sudo`, and `apt`.
